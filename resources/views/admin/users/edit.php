...
<style>
.crm-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1rem; }
.crm-card-title { margin: 0 0 1rem; font-size: 1.05rem; }
.crm-list { margin: 0; padding-left: 1.2rem; color: #334155; }
.crm-kv { display: grid; grid-template-columns: minmax(140px, 180px) 1fr; gap: .4rem .75rem; margin: 0; }
.crm-kv dt { color: #64748b; font-weight: 600; }
.crm-kv dd { margin: 0; color: #0f172a; }
...
<div class="page-header">
    ...
    <a href="/admin/users" class="btn btn-outline" style="border: 1px solid #ddd; color: #334155;"><i class="fas fa-arrow-left"></i> <?php echo __('crm_back'); ?></a>
</div>
...
<div class="crm-grid">
    <div class="card"><div class="card-body">
        ...
        <dl class="crm-kv">
            <dt><?php echo __('crm_profile_name'); ?></dt><dd><?php echo htmlspecialchars($fullName); ?></dd>
            <dt><?php echo __('crm_profile_email'); ?></dt><dd><?php echo htmlspecialchars((string) ($user['email'] ?? '')); ?></dd>
            <dt><?php echo __('crm_profile_phone'); ?></dt>
            <dd><?php echo htmlspecialchars((string) ($user['phone'] ?? '—')); ?>
                <?php if (!empty($user['phone'])): ?><a href="tel:<?php echo urlencode((string) $user['phone']); ?>" class="btn btn-outline" style="margin-left: .5rem;"><?php echo __('crm_call_now'); ?></a><?php endif; ?>
            </dd>
            <dt><?php echo __('crm_profile_registered_at'); ?></dt><dd><?php echo htmlspecialchars($registeredAt); ?></dd>
            <dt><?php echo __('crm_location_primary'); ?></dt><dd><?php echo htmlspecialchars((string) ($crmData['locations']['primary'] ?? '—')); ?></dd>
            <dt><?php echo __('crm_location_additional'); ?></dt>
            <dd><ul class="crm-list"><?php foreach (($crmData['locations']['additional'] ?? []) as $address): ?><li><?php echo htmlspecialchars((string) $address); ?></li><?php endforeach; ?></ul></dd>
            <dt><?php echo __('crm_user_group'); ?></dt><dd><?php echo htmlspecialchars((string) ($groupLabels[$userGroup] ?? __('crm_user_group_regular'))); ?></dd>
        </dl>
    </div></div>
    ...
    <div class="card"><div class="card-body">
        ...
        <dl class="crm-kv">
            <dt><?php echo __('crm_stats_orders_count'); ?></dt><dd><?php echo (int) ($crmData['stats']['orders_count'] ?? 0); ?></dd>
            <dt><?php echo __('crm_stats_ltv'); ?></dt><dd><?php echo number_format((float) ($crmData['stats']['ltv'] ?? 0), 2, '.', ' '); ?></dd>
            <dt><?php echo __('crm_stats_avg_check'); ?></dt><dd><?php echo number_format((float) ($crmData['stats']['average_check'] ?? 0), 2, '.', ' '); ?></dd>
            <dt><?php echo __('crm_stats_last_order'); ?></dt><dd><?php echo htmlspecialchars($lastOrderAt); ?></dd>
        </dl>
    </div></div>
    ...
    <div class="card"><div class="card-body">
        ...
        <ul class="crm-list" style="margin-bottom: 1rem;">
            <?php foreach (($crmData['orders'] ?? []) as $order): ?>
                <li><a href="/admin/orders/details/<?php echo (int) ($order['id'] ?? 0); ?>">#<?php echo (int) ($order['id'] ?? 0); ?></a> — <?php echo htmlspecialchars((string) ($order['created_at'] ?? '')); ?>, <?php echo number_format((float) ($order['total'] ?? 0), 2, '.', ' '); ?>, <?php echo htmlspecialchars((string) ($order['status'] ?? '')); ?></li>
            <?php endforeach; ?>
        </ul>
        ...
        <ul class="crm-list" id="crm-live-cart" style="margin-bottom: 1rem;">
            <?php foreach (($crmData['live_cart'] ?? []) as $item): ?>
                <li><?php echo htmlspecialchars((string) ($item['product_name'] ?? '')); ?> ×<?php echo (int) ($item['quantity'] ?? 0); ?></li>
            <?php endforeach; ?>
        </ul>
        ...
        <ul class="crm-list"><?php foreach (($crmData['wishlist'] ?? []) as $wishItem): ?><li><?php echo htmlspecialchars((string) ($wishItem['product_name'] ?? '')); ?></li><?php endforeach; ?></ul>
    </div></div>
...
<div class="card" style="margin-top: 1rem; max-width: 760px;"><div class="card-body">
    ...
    <form action="/admin/users/update/<?php echo (int) $user['id']; ?>" method="POST" id="crm-user-edit-form">
        ...
        <div class="form-group"><label for="phone"><?php echo __('crm_profile_phone'); ?></label><input class="form-control" type="text" id="phone" name="phone" value="<?php echo htmlspecialchars((string) ($user['phone'] ?? '')); ?>"></div>
...
(function () {
    ...
    const postForm = (url, body) => fetch(url, {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body});

    async function refreshCart() {
        const response = await fetch('/admin/users/live-cart/' + userId, {headers: {'X-Requested-With': 'XMLHttpRequest'}});
        const data = await response.json();
        if (!data.success || !liveCart) return;
        liveCart.innerHTML = '';
        data.items.forEach((item) => {
            const li = document.createElement('li');
            li.textContent = (item.product_name || '') + ' ×' + Number(item.quantity || 0);
            liveCart.appendChild(li);
        });
    }

    setInterval(refreshCart, refreshSeconds * 1000);
    ...
    const emailSubscription = document.getElementById('crm-subscribe-email');
    ...
    const sendEmailButton = document.getElementById('crm-send-email');
    if (sendEmailButton) {
