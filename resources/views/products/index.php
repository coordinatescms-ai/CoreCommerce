...
<div style="display:grid; grid-template-columns: 300px 1fr; gap: 1.25rem; align-items: start;">
    <aside>
        ...
        <?php
        /**
         * Той самий слот, що й на сторінці категорії — банер у лівому
         * сайдбарі під деревом категорій. Спільна назва хука дозволяє
         * плагіну банерів налаштувати одне правило одразу для обох сторінок.
         */
        do_action('catalog.sidebar.after_filters', null);
        ?>
    </aside>

    <section>
        <?php
        /** Слот для банера над сіткою товарів (як на сторінці категорії). */
        do_action('catalog.content.before_products', null);
        ?>
...
<?php
/** Слот для банера внизу сторінки (як на сторінці категорії). */
do_action('catalog.content.after', null);
?>
