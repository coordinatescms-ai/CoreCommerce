...
class Attribute extends Model
{
    ...
    /**
     * Отримати опцію атрибута за значенням
     *
     * @param int $attributeId
     * @param string $value
     * @return array|null
     */
    public static function getOptionByValue($attributeId, $value)
    {
        $result = self::query(
            "SELECT * FROM attribute_options WHERE attribute_id = ? AND LOWER(value) = LOWER(?) LIMIT 1",
            [(int) $attributeId, trim((string) $value)]
        );
